...
public class ExcelToolsExport {
     ...
     * </pre>
     */
    private static Map<String, List<Field>> exportColMap = new ConcurrentHashMap<String, List<Field>>();

    /**
     * 描述：将List对象集合 转化为 byte 数组，生成一张sheet的byte数组源 <br>
     *
     * @param list      : 对象集合数据源
     * @param sheetName ：要生成的 sheet 的名称
     * @return byte数组
     * @throws Exception
     ...
     */
    public final static <T> byte[] createExcelExport(List<T> list, String sheetName)
            throws Exception {
        checkValidate(list);
     ...
     *
     * @param map : 封装的sheet的数据源集合（Map的key为sheet的名称，value为填充的Bean对象集合）
     * @return byte数组
     * @throws Exception
     * @method ：createExcelExport<br>
     * @author ：Andy.wang<br>
     ...
     */
    public final static byte[] createExcelExport(Map<String, List<?>> map)
            throws Exception {
        if (null != map && map.size() > 0) {
            Workbook wb = createWorkbook();

            Iterator<String> it = map.keySet().iterator();
            SXSSFSheet sheet;// 生成的 sheet 对象
            String sheetName;// sheet 名称
            List<?> list; // sheet 数据源
            while (it.hasNext()) {
                sheetName = it.next();
     ...
     *
     * @param sheet ：创建的 Sheet对象
     * @param list  ：对象集合数据源
     * @param wb    ：创建的WorkBook 工作薄对象
     * @throws Exception
     * @method ：setExcelLines<br>
     * @author ：Andy.wang<br>
     * @createDate ：2015年12月2日下午3:23:25 <br>
     */
    private final static <T> void setExcelLines(SXSSFSheet sheet, List<T> list,
     ...
     */
    private final static <T> void obj2Cell(Row row, T t, Workbook wb)
            throws Exception {

        ExcelExportCol excelExportCol;
        Cell cell; // 单元格
        int cols;// 单元格 列索引
        Object value;// 单元格内容（反射获取的属性值）
     ...
     */
    private final static Workbook createWorkbook() {
     ...
     */
    private final static byte[] getByteFormWb(Workbook wb) throws Exception {
        if (null != wb) {
     ...
     *
     * @param cla ：Excel 的载体实体类
     * @return
     * @method ：getHeaderRow<br>
     * @author ：Andy.wang<br>
     * @createDate ：2015年12月2日下午2:35:20 <br>
     */
    private final static <T> int getHeaderRow(Class<T> cla) {
     ...
     */
    private final static <T> int getLineStartRow(Class<T> cla) {
    ...
    private final static <T> List<Field> getExcelExportColAnnoFields(
            Class<T> cla) {
        if (!exportColMap.containsKey(cla.getName())) {
            ...
            exportColMap.put(cla.getName(), list);
        }
        return exportColMap.get(cla.getName());
    }
     ...
     */
    private final static boolean checkValidate(List<?> list) throws Exception {
